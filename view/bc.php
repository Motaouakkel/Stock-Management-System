...
input[type=button], input[type=submit], input[type=reset] {
  ...
  cursor: pointer;
		...
		$sql1="SELECT id_fornecedor,nome
		from fornecedores";
	...
	<body>
		<div class="container">
		<br />
			<h2>Bon de Commande <i class="fa fa-tags"></i></h2>
			<div class="row">
				<div class="col-sm-4">
				
					<form id="bc" method="post" action="../classes/bc.php" enctype="multipart/form-data">
					<br />
					<label>N° Référence : <i class="fa fa-tag"></i></label>
						<input type="text" class="form-control input-sm" id="reef" name="reef" placeholder="00/0000">
						
						<label>Date <i class="fa fa-tag"></i></label>
						<input type="text" class="form-control input-sm" id="date" name="date" placeholder="00/00/0000">
						<label>Fournisseur</label>
						<select class="form-control input-sm" id="fourniSelect" name="fourniSelect">
							<option value="B">Séléctionner Fournisseur</option>
							<?php while($mostrar1=mysqli_fetch_row($result1)): ?>
								<option value="<?php echo $mostrar1[0] ?>"><?php echo $mostrar1[1]; ?></option>
							<?php endwhile; ?>
							</select>
							<br />
						<label>Fiche BC <i class="fa fa-images"></i></label>
			...
			<div class="modal-dialog modal-sm" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button id="btnAtualizarProduto" type="button" class="btn btn-warning" data-dismiss="modal">Editarsdfdf</button>
		...
		function eliminarProduto(idProduto){
			alertify.confirm('Voulez vous Supprimer cette ligne', function(){ 
				$.ajax({
					...
					url:"../procedimentos/produtos/subc.php",
					success:function(r){
						if(r==1){
